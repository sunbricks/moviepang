...
<%-- 	<input type="button" onclick="location.href='${pageContext.request.contextPath }/productList.ko';" value="상품목록보기" /> --%>
	<form action="${pageContext.request.contextPath }/proDetailInsert.ko" method="post" id="dataTransferForm" enctype="multipart/form-data">
		<textarea name="pdetail" id="ir1" rows="10" cols="100" style="width:766px; height:412px; display:none;"></textarea>
		썸네일 이미지 등록: <input type="file" name="pthumbnailimg" id="pthumbnailimg">
		<input type="text" name="pname" id="pname" value="${pOne.pname }" placeholder="상품명 입력">
		<input type="hidden" name="ppno" id="ppno" value="${pOne.pno }">
<%-- 		<input type="number" name="pprice" id="pprice" value="${pOne.pprice }" placeholder="상품금액 입력"> --%>
...
window.onload = function(){
	document.querySelector("#dataTransferForm #ir1").innerText = '${pOne.pdetail }';
	
<%-- 	let str = '<%=request.getParameter("msg")%>'; --%>
// 	if( str == "ok") {
// 		var httpReq = new XMLHttpRequest();
// 		httpReq.open("GET", '${pageContext.request.contextPath }/WEB-INF/pno${pOne.pno}.jsp', false);
// 		httpReq.onreadystatechange = function(){
// 			if( httpReq.readyState == 4 && httpReq.status == 200  ) {
// 				var fileData = httpReq.responseText;
// 				console.log('fileData: ',fileData);
// 				document.querySelector("#dataTransferForm #ir1").innerText = fileData;
// 			}
// 		};
// 		httpReq.send();
// 	}
...
if(window.frameElement){
	...
	function submitContents() {
		...
		if (jindo.$("ir1").form.pname.value==''){
			alert('상품명을 입력해주세요.');
			jindo.$("ir1").form.pname.focus();
			return;
		}
		if (jindo.$("ir1").form.pprice.value==''){
			alert('상품금액을 입력해주세요.');
			jindo.$("ir1").form.pprice.focus();
			return;
		}
		// 20240503_ym 추가 종료		
